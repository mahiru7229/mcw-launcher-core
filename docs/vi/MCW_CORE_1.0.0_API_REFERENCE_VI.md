# Tài liệu API MCW Core 1.0.0 (Tiếng Việt)

Tài liệu này được chuẩn hóa lại từ **wheel stable `mcw_core-1.0.0-py3-none-any.whl`** và dùng làm bản tham chiếu API tiếng Việt cho các lập trình viên muốn xây launcher riêng bằng MCW Core.

## Phạm vi tài liệu

Tài liệu tập trung vào **public API** có thể dùng để xây ứng dụng: 

- `mcw_core`
- `mcw_core.api.*`

Không khuyến khích phụ thuộc trực tiếp vào các module tương thích nội bộ như `src.*`, vì đó không phải biên public API ổn định lâu dài.

## Cách đọc tài liệu

- Xem `MCWCore` như facade cấp cao để điều phối các tác vụ chính.
- Xem `mcw_core.api.*` như tập các service và manager chi tiết hơn.
- Với các hàm chạy lâu, hãy chú ý `ProgressEvent`, callback `on_progress`, cùng cơ chế pause / resume / cancel.
- Với launch game, lưu ý `launch()` **trả về khi process Minecraft đã được tạo**, còn callback `on_exit` sẽ được gọi **sau khi game đóng**.

## Ví dụ import an toàn
